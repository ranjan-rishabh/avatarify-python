...
class PredictorLocalNvidia:
    def __init__(self, config_path, checkpoint_path, relative=False, adapt_movement_scale=False, device=None, enc_downscale=1):
        ...
        self.enc_downscale = enc_downscale
        self.estimate_jacobian = False

    def load_checkpoints(self, gen='spade', cpu=False):
    ...
    def set_source_image(self, source_image):
        ...
        self.kp_source = self.keypoint_transformation(self.he_source)
    ...
    def predict(self, driving_frame):
        ...
        with torch.no_grad():
            ...
            if self.kp_driving_initial is None or self.kp_canonical is None:
                he_driving_initial = self.he_estimator(driving)
                self.kp_driving_initial = self.keypoint_transformation(he_driving_initial)
                self.start_frame = driving_frame.copy()
                self.start_frame_kp = self.get_frame_kp(driving_frame)

            he_driving = self.he_estimator(driving)
            kp_driving = self.keypoint_transformation(he_driving)
            kp_norm = normalize_kp(kp_source=self.kp_source, kp_driving=kp_driving,
                                kp_driving_initial=self.kp_driving_initial, use_relative_movement=self.relative,
                                use_relative_jacobian=self.estimate_jacobian, adapt_movement_scale=self.adapt_movement_scale)
    ...
    def keypoint_transformation(self, he, free_view=False, yaw=0, pitch=0, roll=0):
        kp = self.kp_canonical['value']
        if not free_view:
        ...
        if self.estimate_jacobian:
            jacobian = self.kp_canonical['jacobian']
